...
- [ ] **lane adopts & consumes the host network plane (ADR-0003)** — owner-directed 2026-06-17:
  *"direct lane to adopt-consume and rust port the current box's NM so meta is truly portable."*
  ...
  - **P0 Adopt (read-only):** serde model (superset of netplan v2) + `lane net adopt` (live host → model) + round-trip the snapshot. No host mutation.
    ...
    - [x] **P0b** — `lane net adopt` live host reader (sources from `nmcli`, unprivileged + secret-safe; NOT `/etc/netplan` which is root-only). `hostnet` feature (default-off, no new deps). Sanitizing: secrets→`SecretRef` driven by `key-mgmt` (nmcli masks ALL wifi secrets as `<hidden>` regardless of existence — OWE APs get no spurious ref). Round-trips the LIVE box's cognitum-seed unit to the snapshot shape. Shipped: PR #57 (425 default + 13 hostnet tests green; verified live + guard-clean). ARCHITECTURE.md `## src/net` section added.
  - **P0 "Adopt" milestone COMPLETE** (read-only half of the epic — the owner's "Start at P0"). Next is P1 (render = host-mutating).
  - **DECONFLICT: LOCKED** — boundary with `network-control` is by LAYER not device (network-control=off-host fabric Omada/switch/AP/VLAN/VPN; lane=on-host netplan-NM plane, single writer). ADR-0003 §Deconfliction (merged #56). P1 unblocked.
  - **P1 Render:** `lane net apply` for the netplan-NM renderer; additive reconcile (never flush unowned addrs). Feature-gated, dry-run-default, fail-closed. Split:
    - [x] **P1a** (autonomous) — render engine SHIPPED: PR #59. `reconcile(desired,current)->ReconcilePlan`; `NmcliOp` has ONLY Add/Modify (NO delete/flush variant — additive is TYPE-level, can't flush unowned). Keys by match+name not UUID. `--dry-run` DEFAULT, `--apply` explicit opt-in (conflicts), fail-closed via `osutil::run_privileged`. Both pre-decisions landed: (1) runtime-bridge exclusion applied SYMMETRICALLY to desired+current (a review-caught asymmetry bug was fixed + regression-tested); (2) passthrough normalization (bookkeeping keys normalized; significant keys still diff). Proven LIVE: whole-host adopt|apply --dry-run => empty plan (idempotent). 435 default + net::apply tests; verified + guard-clean.
    - [ ] **P1b** (HUMAN WALL — remains) — live MUTATING apply (sudo nmcli/netplan write) + cognitum-seed durability across **carrier bounce + reboot**. `--apply` is shipped + fail-closed; its LIVE execution needs sudo + reboot → NEEDS-HUMAN, do NOT fake green. Verify via REAL nmcli/ip/iptables, NEVER `lane doctor` (FlexNetOS/lane#5).
  - **P2 Portability:** [x] SHIPPED — PR #60 (arming). (A) in-repo per-host profiles: `lane net profile save|list|show` (capture, runtime-stripped, secret-safe, name-traversal-guarded) + `lane net apply --host <name>` (reproduce). (B) systemd-networkd renderer for non-NM boxes: pure `render_networkd`, backend by `renderer` field / `--renderer networkd`; NO-DOWNGRADE enforced STRUCTURALLY (exhaustive-destructure field audit — a new model field is a compile error until rendered-or-gapped; review caught + fixed wakeonlan/nameservers/on_link silent drops). 458 default + 44 hostnet net tests; verified + guard-clean. Live-mutating networkd file writes share the P1b human wall.
  - **P3 env-ctl seam:** [~] KICKED OFF — weave #127 to env-ctl with PARITY EVIDENCE (lane adopt round-trips + apply renders the cognitum-seed unit, self-apply empty). Proposed STAGED no-downgrade migration (env-ctl keeps the trigger/USB-unlock ownership; lane owns rendering; PR #115 keeps working until byte-parity proven). Awaiting env-ctl's choice: committed host-profile vs `lane net apply` call at unlock. Cross-repo + staged → not single-session autonomous; tracked via weave.
