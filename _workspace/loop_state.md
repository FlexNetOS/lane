...
in_flight:
  ...
  next_item: QUEUED — FULL VERIFICATION AUDIT of the shipped W2 work (adversarial; verify all code implementation + every task done properly, no fabricated greens, no-downgrade honored). See `_workspace/AUDIT-KICKOFF.md`. Drive verification-engineer + rust-native-guardian (optionally /harness:code-research); write `_workspace/AUDIT-REPORT-<date>.md`. THEN the non-autonomous remainder: (a) P1b = live `--apply` + cognitum-seed reboot durability (HUMAN WALL). (b) P3 env-ctl seam (CROSS-REPO, weave #127).
