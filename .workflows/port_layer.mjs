...
// args = { layer: string, modules: [{ name, go_sources:[string], rust_files:[string], notes:string }] }
...
CRITICAL RULES:
- The binding API contract is /home/drdave/Desktop/lane/ARCHITECTURE.md — READ IT FIRST and match
...
- Original Go source (read-only reference) lives under /home/drdave/Downloads/slim-extract/slim-main/.
...
${m.go_sources.map(s => `  - /home/drdave/Downloads/slim-extract/slim-main/${s}`).join('\n')}

Write/overwrite exactly these Rust file(s) (you own them, including mod.rs):
${m.rust_files.map(s => `  - /home/drdave/Desktop/lane/${s}`).join('\n')}
