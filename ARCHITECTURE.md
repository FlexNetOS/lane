...
Top-level: on error print `\nError: {e}` to stderr (red "Error:") and exit 1. `version` prints `lane {VERSION}`.
...
`start, stop, up, down, list, logs, share, doctor, login, logout, domain(add/list/verify/remove), uninstall, upgrade`.
